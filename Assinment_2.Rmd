...
```
*The mean and median of keyword "mask"is 37.37 and 31 respectively while that of the "quarantine" is 10 and 8. The variance for the two keywords;mask and quarantine are 509 and 66.04 respectively
...
```{r}
 #relationship between median household income and search popularity
#scatterplot <- qplot(x = hh_income, y = mean_search_crime + mean_search_loans, data = merged_data)
scatterplot <- qplot(x = hh_income, y = mean_search_crime, data = merged_data)
scatterplot
```

```{r}

# Group by income_diff and calculate the mean search popularity for "crime" and "loans"
result <- merged_data %>%
  group_by(income_diff) %>%
  summarize(mean_search_popularity_crime = mean(crime),
            mean_search_popularity_loans = mean(loans))
# Recode the binary_column
merged_data <- merged_data %>%
  mutate(income_diff = recode(income_diff, `1` = "Yes", `0` = "No"))
...
# Scatterplot using qplot for crime
Scatterplot<-qplot(x = hh_income, y = crime, data = merged_data, color = "red", shape = I(16)) +
  geom_point(aes(x = hh_income, y = crime, color = "blue", shape = I(17))) +
  labs(x = "hh_income", 
       y = "mean_search_popularity_crime",
       color = "Terms",
       shape = "Terms",
       title = "Relationship between Median Income and Google Trends Popularity") +
  scale_color_manual(values = c("red", "blue"), breaks = c("red", "blue"), labels = c("Crime", "Loans")) +
  scale_shape_manual(values = c(16, 17), breaks = c(16, 17), labels = c("Crime", "Loans")) +
  theme_minimal()
Scatterplot
